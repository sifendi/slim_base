...
$(document).ready(function() {
	...
	$("input[name=status_karyawan]").enterKey(function (e) {
		e.preventDefault();
		$(".btn_save").click();
	});
	...
	table = $("#datatable-responsive").DataTable({
		...
		"buttons": [
			...
			},
			{
				...
				title: function () { return 'Master Karyawan';},
				exportOptions: {
					/* Show column */
			...
			"dataSrc": function(json) {
				...
				if (json.success === true) {
					...
					for (var i = 0; i < json.message.data.length; i++) {
						return_data.push({
							0: json.message.data[i].id_karyawan,
							1: json.message.data[i].no,
							2: json.message.data[i].nama_karyawan,
							3: json.message.data[i].alamat_karyawan,
							4: json.message.data[i].tgllahir_karyawan,
							5: json.message.data[i].status_karyawan,
						})
					}
					return return_data;
				} else {
	...
	$(tbl).on( 'click', '#btEdit', function () {
		...
		/* Set Edit Form Value */
		$("input[name=nama_karyawan]").val(data[2]);
		$("input[name=alamat_karyawan]").val(data[3]);
		$("input[name=tgllahir_karyawan]").val(data[4]);
		$("input[name=status_karyawan]").val(data[5]);
		$('.btn_save').html('<i class="fa fa-save"></i> Update');
		$('.modal-title').html('Edit Master Karyawan');
...
function btn_add() {
	...
	$('.modal-title').html('New Master Karyawan');
