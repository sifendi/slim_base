...
$(document).ready(function() {
	...
	$("input[name=status]").enterKey(function (e) {
		e.preventDefault();
		$(".btn_save").click();
	});
	...
	table = $("#datatable-responsive").DataTable({
		...
		"buttons": [
			...
			},
			{
				...
				title: function () { return 'Master Hobby';},
				exportOptions: {
					/* Show column */
			...
			"dataSrc": function(json) {
				...
				if (json.success === true) {
					...
					for (var i = 0; i < json.message.data.length; i++) {
						return_data.push({
							0: json.message.data[i].id_hobby,
							1: json.message.data[i].no,
							2: json.message.data[i].hobby,
							3: json.message.data[i].deskripsi,
							4: json.message.data[i].status,
						})
					}
					return return_data;
				} else {
	...
	$(tbl).on( 'click', '#btEdit', function () {
		...
		/* Set Edit Form Value */
		$("input[name=hobby]").val(data[2]);
		$("input[name=deskripsi]").val(data[3]);
		$("input[name=status]").val(data[4]);
		$('.btn_save').html('<i class="fa fa-save"></i> Update');
		$('.modal-title').html('Edit Master Hobby');
...
function btn_add() {
	...
	$('.modal-title').html('New Master Hobby');
